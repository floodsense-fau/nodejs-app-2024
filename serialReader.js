/**
 * AI Summary:
 * This script reads distance data from a serial port (e.g., an ultrasonic sensor),
 * parses the incoming data, and periodically sends the latest reading to both a Kafka topic
 * and an InfluxDB time-series database. It uses environment variables for configuration,
 * handles errors for Kafka and serial connections, and ensures a graceful shutdown by flushing
 * any remaining data to InfluxDB. The script is designed for use in a sensor data collection
 * and streaming pipeline.
 *
 * Note: If you use tools like readFileSync elsewhere, it is a synchronous Node.js method
 * for reading the contents of a file directly from disk.
 */
